...
export default function SoupPage () {
  ...
  const soupData = menus.filter((item) => item.category === "soup");

  useEffect(() => {
    setDatas(soupData.slice(page * 9 - 9, page * 9));
  }, [page]);
    ...
    <section ref={sectionRef} className="container mx-auto">
      ...
      <div className="mt-15 mb-24 flex items-center gap-4">
        ...
        <p className="text-xl font-bold">
          {page} / {Math.ceil(soupData.length / 9)}
        </p>
        ...
        <button
          ...
          onClick={() => (setPage(page + 1), scrollToSection())}
          disabled={page >= Math.ceil(soupData.length / 9)}
        >
          <GoArrowRight />
        </button>
      </div>
